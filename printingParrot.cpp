...
void print_message(V v, M m){
	switch(m){
		case Registration: cout<<"A ";
			if(typeid(v)==typeid(Lorry)){cout<<v.get_ppcm()<<"-ppcm ";}
			if(typeid(v)==typeid(dieselCar)){cout<<v.get_axles()<<"-axle ";}
			cout<<v.get_type()<<" ("<<v.get_name()<<") has been registered"<<endl;
			break;
		...
		case Entrance:cout<<"The ";
			if(typeid(v)==typeid(Lorry)){cout<<v.get_ppcm()<<"-ppcm ";}
			if(typeid(v)==typeid(dieselCar)){cout<<v.get_axles()<<"-axle ";}
			cout<<v.get_type()<<" ("<<v.get_name()<<") enters on "<<v.get_date()<<" at "<<v.get_time()<<" hours"<<endl;
			if(v.wascharged()){
